...
«Оспорить основную гипотезу» и сравнение двух GID.
...
необязательный `compare_gid`; возвращает NDJSON activity/result/error.
...
Модель сама выбирает инструменты. Сервер требует профиль, structural evidence
...
не скрытые рассуждения. Максимум два заданных пользователем GID, 16 tool calls,
...
метрики/роли/суммы, ограничения, локальные связи или агрегаты кластера.
...
только GID выбранного узла, явно указанные в вопросе или поле сравнения.
...
в карточке `/api/nodes/{gid}`. Tools и prompt AI PHASE 5 не изменены;
