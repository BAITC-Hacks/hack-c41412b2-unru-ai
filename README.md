...
**Функции заморожены:** локальный pipeline, три CSV, наблюдаемость и очереди, UI с поиском любого GID, объяснимые гипотезы, AI-критик с поиском общего downstream для 1–5 источников, offline-архив реальных AI-ответов, Stability Lite и TOP-N removal. Temporal не реализован. Основной продукт и архив работают без API-ключа и интернета. Ключ нужен только для нового ответа LLM. Формулы ролей/приоритета и три CSV побайтно совпадают с PHASE 1 (`09d2491`).

[Сценарий защиты на 5 минут](docs/DEMO_5_MIN.md) · [Подготовка сдачи](docs/SUBMISSION.md).

**Проверено в чистой копии `336b6e3`:** 85 Python-тестов + 3 JS renderer checks; три CSV совпадают по SHA-256. Первый расчёт после установки — 5.10 с, повторный — 0.47 с. UI, три случайных GID и offline-архив проверены без ключа. [Отчёт](audit/final_reproduction.json), [установленные версии](audit/final_environment.txt).
...
Следующие идеи исследования ещё **не реализованы**: effective last-hop branches, disjoint paths, изменение сортировки запросов, AI Hypothesis Critic, Stability Lite и temporal. Их нельзя показывать как готовые возможности. Предложенные новые формулы ролей и нормализации не перенесены в принятое ядро.
