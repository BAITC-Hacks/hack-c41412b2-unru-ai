...
Python **3.12**, обычный CPU, ориентир 2 GB свободной RAM. Интернет нужен только один раз для установки пакетов. Проверено на macOS arm64 / Python 3.12.14 в отдельном окружении, созданном с нуля; финальная проверка выполняется стандартным pip по командам ниже. Выполнять из корня этого репозитория:
