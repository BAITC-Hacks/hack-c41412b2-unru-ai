...
- Разработка и тестирование с помощью OpenAI Codex. Runtime LLM в PHASE 4A отсутствует, внешние AI-вызовы над данными не выполняются. Предварительные тренировочные проекты в решение не перенесены.
...
q(p) = mass(p) / Σ mass
last_hop_effective_branches = exp(-Σ q(p) * ln(q(p)))
```
