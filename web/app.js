...
function renderPatterns(p) {
  ...
  <details><summary>Отклонения от depth=${p.depth} · ${a.flag_count} сигналов</summary><p class="explanation">Средний ранг одинаковых значений среди узлов того же depth; сигнал при ≥99-м процентиле и положительном значении. Минимум 20 участников. Это не вероятность и не норматив «нормального» поведения. У seed вход неполон; выборка ограничена.</p>
  <table><thead><tr><th>Признак</th><th>Значение</th><th>Процентиль</th></tr></thead><tbody>${Object.entries(a.peers).map(([k,v])=>`<tr><td>${labels[k]}</td><td>${val(v.value)}</td><td>${v.percentile===null?'N/A · '+esc(v.status):fmt(v.percentile*100)+(v.flag?' · СИГНАЛ':'')}<br><small>n=${v.peer_count}</small></td></tr>`).join('')}</tbody></table>
