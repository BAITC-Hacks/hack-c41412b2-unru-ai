...
// Layout only: retain existing rendered facts and handlers, move them into panels.
function organizeInspector() {
  const card=$('node-card'), parts=Array.from(card.children);
  const groups=[['overview','Обзор',[1,2,3,14]],['structure','Структура',[4,5,10,11]],['patterns','Паттерны',[6,7]],['stability','Устойчивость',[8,9]],['data','Данные',[12,13]]];
  const tabs=document.createElement('div');tabs.className='inspector-tabs';tabs.setAttribute('role','tablist');tabs.setAttribute('aria-label','Разделы карточки узла');
  const content=document.createElement('div');content.className='inspector-content';
  groups.forEach(([id,label,indices],index)=>{
    const button=document.createElement('button');button.type='button';button.id='tab-'+id;button.textContent=label;button.setAttribute('role','tab');button.setAttribute('aria-controls','inspect-'+id);button.setAttribute('aria-selected',String(index===0));button.tabIndex=index===0?0:-1;
    const panel=document.createElement('section');panel.id='inspect-'+id;panel.setAttribute('role','tabpanel');panel.setAttribute('aria-labelledby',button.id);panel.hidden=index!==0;
    indices.forEach(i=>panel.append(parts[i]));
    button.addEventListener('click',()=>{
      tabs.querySelectorAll('[role=tab]').forEach(b=>{b.setAttribute('aria-selected',String(b===button));b.tabIndex=b===button?0:-1;});
      content.querySelectorAll('[role=tabpanel]').forEach(p=>p.hidden=p!==panel);content.scrollTop=0;
    });
    button.addEventListener('keydown',event=>{
      const keys=['ArrowLeft','ArrowRight','Home','End'];if(!keys.includes(event.key))return;event.preventDefault();
      const buttons=Array.from(tabs.children),next=event.key==='Home'?0:event.key==='End'?buttons.length-1:(index+(event.key==='ArrowRight'?1:-1)+buttons.length)%buttons.length;
      buttons[next].click();buttons[next].focus();
    });
    tabs.append(button);content.append(panel);
  });
  // Keep the useful request in the Data tab too, without changing its wording.
  content.querySelector('#inspect-data').append(parts[14].cloneNode(true));
  card.append(tabs,content);
}
